...
```
即使没有给函数传递任何参数，产生的输出也都不相同，也就是说`Math.random`并非是一个纯函数。每次执行`Math.random()`,都会生成一个0到1的随机数，所以很显然，你没法用`0.4011148700956255`来替换`Math.random()`，并且不改变程序的含义。当我们要求计算机生成一个随机数时，通常意味着我们想要的是一个与最后一次的到的数不同的结果。如果骰子的每一边印的都是同样的数字，那么骰子又有什么意义呢？     
